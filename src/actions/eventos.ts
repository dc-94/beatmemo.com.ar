...
export async function upsertEvento(formData: FormData, id?: string): Promise<ActionResponse> {
  try {
    ...
    if (!roleData || !['SUPERADMIN', 'CONTENT_ADMIN'].includes(roleData.role)) {
      logAdminAction(id ? 'UNAUTHORIZED_UPDATE_EVENT_ATTEMPT' : 'UNAUTHORIZED_CREATE_EVENT_ATTEMPT', 'eventos', { email: user.email, target_id: id });
    ...
    logAdminAction(id ? 'UPDATE_SHOW' : 'CREATE_SHOW', 'eventos', {
      user_id: user.id,
      email: user.email,
      show_id: savedShow?.id,
      show_titulo: savedShow?.titulo
    });
...
export async function deleteEvento(showId: string): Promise<ActionResponse> {
  try {
    ...
    if (!roleData || !['SUPERADMIN', 'CONTENT_ADMIN'].includes(roleData.role)) {
      logAdminAction('UNAUTHORIZED_DELETE_EVENT_ATTEMPT', 'eventos', { email: user.email, show_id: showId });
    ...
    logAdminAction(roleData.role === 'SUPERADMIN' ? 'HARD_DELETE_SHOW' : 'SOFT_DELETE_SHOW', 'eventos', {
      user_id: user.id,
      email: user.email,
      show_id: showId
    });
